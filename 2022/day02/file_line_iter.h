...
#ifndef YEAR2022_DAY02_FILE_LINE_ITER_H
#define YEAR2022_DAY02_FILE_LINE_ITER_H
...
enum {
	...
	FILE_LINE_ITER_TMP_BUF_SIZE     = 2048,
};
...
static inline int file_line_iter_next(struct file_line_iter* const iter, struct file_line_iter_next* const next) {
	...
	do {
		char tmp_buf[FILE_LINE_ITER_TMP_BUF_SIZE];
		const size_t readc = fread(tmp_buf, sizeof *tmp_buf, (sizeof tmp_buf / sizeof *tmp_buf), iter->file_stream);

		eof = feof(iter->file_stream);
		err = ferror(iter->file_stream);

		size_t nl_index = (size_t)-1;
		...
		if (nl_index != (size_t)-1) {
			if (dynamic_sso_string_init_from_buf(&(internal_next.line), tmp_buf, nl_index) != 0) {
				return 1;
			}

			if (dynamic_string_append(&(iter->backlog), (tmp_buf + nl_index + 1), (readc - nl_index - 1)) != 0) {
				int errno_tmp = errno;
				dynamic_sso_string_destroy(&(internal_next.line));
				errno = errno_tmp;
				return 1;
			}
			...
			if (dynamic_string_append(&(iter->backlog), tmp_buf, readc) != 0) {
				int errno_tmp = errno;
				dynamic_sso_string_destroy(&(internal_next.line));
				errno = errno_tmp;
				return 1;
...
#endif /* YEAR2022_DAY02_FILE_LINE_ITER_H */
