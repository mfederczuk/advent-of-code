...
#ifndef YEAR2022_DAY03_DYNAMIC_STRING_H
#define YEAR2022_DAY03_DYNAMIC_STRING_H
...
static inline int dynamic_string_append_char(struct dynamic_string* const s, const char ch) {
	if (dynamic_string_reserve(s, (s->size + 1)) != 0) {
		return 1;
	}
...
static inline int dynamic_string_append_buf(struct dynamic_string* const s,
                                            const char* const buf,
                                            const size_t buf_size) {
...
#endif /* YEAR2022_DAY03_DYNAMIC_STRING_H */
