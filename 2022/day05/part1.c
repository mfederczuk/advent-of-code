...
#include "utils/rev_string_iter.h"
...
int aoc_year2022_day05_main(const const_cstr_t argv0, const const_cstr_t input_file_pathname) {
	...
	do {
		...
		struct rev_string_iter rev_iter =
			rev_string_iter_create(
				(dynamic_sso_string_data_ptr(src_crates_stack) + src_crates_stack_begin_index),
				crates_count
			);

		do {
			char* const ch = rev_string_iter_next(&rev_iter);

			if (ch == NULL) {
				break;
			}

			if (dynamic_sso_string_append_char(dest_crates_stack, *ch) != RET_STATUS_SUCCESS) {
				fail_reason = FAIL_REASON_FAILURE;
				goto fail;
			}
		} while (1);
