...
static bool size_t_closed_range_overlaps_with(const struct size_t_closed_range a, const struct size_t_closed_range b) {
	return (((a.begin >= b.begin) && (a.begin <= b.end)) ||
	        ((b.begin >= a.begin) && (b.begin <= a.end)));
}

int aoc_main(const char* const input_file_pathname) {
	size_t range_overlaps_count = 0;
	...
	do {
		...
		if (size_t_closed_range_overlaps_with(elf1_range, elf2_range)) {
			++range_overlaps_count;
		}
